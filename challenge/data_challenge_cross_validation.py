...
if __name__ == '__main__':
    ...
    # Cross validate weeks 1-7
    for i in range(1, 7):
        setlist = []
        # load previous test weeks
        if i != 1: setlist.append(pd.read_csv("week_1_test_data.csv"))
        if i != 2: setlist.append(pd.read_csv("week_2_test_data.csv"))
        if i != 3: setlist.append(pd.read_csv("week_3_test_data.csv"))
        if i != 4: setlist.append(pd.read_csv("week_4_test_data.csv"))
        if i != 5: setlist.append(pd.read_csv("week_5_test_data.csv"))
        if i != 6: setlist.append(pd.read_csv("week_6_test_data.csv"))
        #if i != 7: setlist.append(pd.read_csv("week_7_test_data.csv"))
        test_set1_7 = pd.concat(setlist)
        test_set1_7 = test_dates(test_set1_7)
        features = design_matrix(test_set1_7)
        ...
        setlabels =[]
        if i != 1: setlabels.append(pd.read_csv("week_1_labels.csv")['cancel'])
        if i != 2: setlabels.append(pd.read_csv("week_2_labels.csv")['cancel'])
        if i != 3: setlabels.append(pd.read_csv("week_3_labels.csv")['cancel'])
        if i != 4: setlabels.append(pd.read_csv("week_4_labels.csv")['cancel'])
        if i != 5: setlabels.append(pd.read_csv("week_5_labels.csv")['cancel'])
        if i != 6: setlabels.append(pd.read_csv("week_6_labels.csv")['cancel'])
        #if i != 7: setlabels.append(pd.read_csv("week_7_labels.csv")['cancel'])
        
        true_labels1_7 = pd.concat(setlabels)

        #load test data for week 7
        test_set8 = pd.read_csv(f"week_{i}_test_data.csv")
        true_labels = pd.read_csv(f"week_{i}_labels.csv")['cancel']
        test_set8 = test_dates(test_set8)
        #bookingdates = test_set5['booking_datetime'].copy(deep=True)
        #checkoutdates = test_set5['checkout_date'].copy(deep=True)
        test_matrix = design_matrix(test_set8)

        # align one-hots
        features, test = features.align(test_matrix, join='inner', axis=1)
        ...
        classifier = train_classifier(features, true_labels1_7)

        # predict
        scaler = StandardScaler().fit(features)
        test = scaler.transform(test)
        ##final_pred = regression_classifier(bookingdates, checkoutdates, test, regressor1, regressor2)
        ##evaluation = pd.read_csv('316493758_208770149_318031010.csv')
        final_pred = pd.DataFrame({'prediction': classifier.predict(test)})

        #evaluate
        print(f"evaluation for set {i}:", sklearn.metrics.f1_score(true_labels, final_pred))
        f1 += sklearn.metrics.f1_score(true_labels, final_pred)
        #print("evaluation: ", classification_report(true_labels6, final_pred))

        # prepare prediction for submission
        #final_pred.to_csv("316493758_208770149_318031010.csv", index=False)
    print("overall f1 performance: ", f1/7)
